...
def load_mat_G(path):
    ...
    for m in range(0, len(G['samples_type'])):
        ...
        for i in range(0, len(G['samples_type'][m])):
            ...
            out_G['samples_type'][m][i]._I = copy.deepcopy(G['models'][i]['I'])
            out_G['samples_type'][m][i]._fixed_parameter= copy.deepcopy(G['models'][i]['parameters'])
            out_G['samples_type'][m][i]._epsilon= copy.deepcopy(G['models'][i]['epsilon'])
            out_G['samples_type'][m][i]._blur_sigma = copy.deepcopy(G['models'][i]['blur_sigma'])
            out_G['samples_type'][m][i]._affine_transformation = np.array(copy.deepcopy(G['models'][i]['A']))
            out_G['samples_type'][m][i]._num_strokes = copy.deepcopy(G['models'][i]['ns'])
            out_G['samples_type'][m][i]._prob_img = copy.deepcopy(G['models'][i]['pimg'])
            out_G['samples_type'][m][i]._ink_off_page = copy.deepcopy(G['models'][i]['ink_off_page'])

            out_G['samples_type'][m][i]._motor = []
            for j in range(len(G['models'][i]['motor'])):
                out_G['samples_type'][m][i]._motor.append([])
                for k in range(len(G['models'][i]['motor'][j])):
                    out_G['samples_type'][m][i]._motor[j].append(copy.deepcopy(G['models'][i]['motor'][j][k]['val']))

            out_G['samples_type'][m][i]._motor_warped = []
            for j in range(len(G['models'][i]['motor_warped'])):
                out_G['samples_type'][m][i]._motor_warped.append([])
                for k in range(len(G['models'][i]['motor_warped'][j])):
                    out_G['samples_type'][m][i]._motor_warped[j].append(copy.deepcopy(G['models'][i]['motor_warped'][j][k]['val']))

            out_G['samples_type'][m][i]._cache_grant_curent = copy.deepcopy(G['models'][i]['ink_off_page'])
